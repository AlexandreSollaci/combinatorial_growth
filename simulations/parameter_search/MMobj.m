function F = MMobj(guess)

    display('starting to evaluate guess');

    etaH = guess(1); % NT step size
    etaM = guess(2); % NC step size
    etaL = 0; % Refinement step size
    ttau = guess(3); % shape parameter for ideas distribution
    llambda = guess(4); % alpha (cost) has weibull distribution with scale parameter lambda ...
    kkappa = guess(5); % and shape parameter kappa
    xxi = guess(6);   % 1/xi is the fraction of feasible combinations

    nrofinv = guess(7); % number of inventors

    ggamma = .6; % match labor share
...
%         nroffirms = mean(JGrowth) / 0.02;
    ...
    display(['ended evaluating guess, F = ' num2str(F)])
