...
for s = 1:num_simul
    seed = 1000*rand;
    model_plain = simulate_model(params, Tbase, beg_year, beg_inv, Mmat0, patmat0, state0, [0 0], seed);
    ...
    model_subs_0 = simulate_model(params, Tsubs, beg_year+Tbase, nrofinv, Mmat, patmat1, state, [0 0], seed);
    ...
    for v = 1:length(subsidy)
        ...
        model_subs_1 = simulate_model(params, Tsubs, beg_year+Tbase, nrofinv, Mmat, patmat1, state, subsidy(v,:), seed);
...
figure(4)
plot(patent_shares(:,1), patent_shares(:,2), 'k', patent_shares(:,1),patent_shares(:,3), '--k', patent_shares(:,1),patent_shares(:,4), '-^k') 
xlim([beg_year, beg_year+Tmax])
xlabel('Year')
...
Delta_welfareNT = (welfareNT - repmat(welfare,1,2))./repmat(welfare,1,2);
Delta_welfareNC = (welfareNC - repmat(welfare,1,2))./repmat(welfare,1,2);
Delta_welfareBOTH = (welfareBOTH - repmat(welfare,1,2))./repmat(welfare,1,2);
